package com.mikufans.manage.web.auth;
...
public class AuthController
{
    ...
    public ModelAndView toPage(Permission permission)
    {
    ...
    {
        logger.debug("权限列表！");
        ModelAndView modelAndView = new ModelAndView("/auth/permList");
        try
        {
            List<Permission> permissions = authService.permList();
            logger.debug("权限列表查询=permList:" + permissions);
            modelAndView.addObject("permList", permissions);
            modelAndView.addObject("msg", "ok");
        } catch (Exception e)
        {
            e.printStackTrace();
